...
static void SetBaudrate(UART_HandleTypeDef *huart, uint32_t baudrate)
{
  ...
  {
    huart->Instance->BRR = UART_DIV_SAMPLING8(pclk, huart->Init.BaudRate); //поменяла BRR на DIV. Во всех источниках пишут так
  }
  else
  {
    huart->Instance->BRR = UART_DIV_SAMPLING16(pclk, huart->Init.BaudRate); //поменяла BRR на DIV
  }
}
